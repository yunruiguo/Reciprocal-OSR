...
cifar100_splits = [['fox', 'baby', 'whale', 'elephant', 'crocodile', 'squirrel', 'hamster', 'mouse', 'turtle', 'dolphin', 'trout',
                    'snake', 'bee', 'beetle', 'lion', 'dinosaur', 'boy', 'butterfly', 'shrew', 'cattle', 'cockroach', 'girl', 'crab',
                    'rabbit', 'kangaroo', 'leopard', 'lobster', 'chimpanzee', 'raccoon', 'otter', 'lizard', 'seal', 'beaver', 'worm',
                    'wolf', 'bear', 'possum', 'aquarium_fish', 'camel', 'caterpillar', 'man', 'porcupine', 'woman', 'spider', 'ray',
                    'flatfish', 'tiger', 'shark', 'skunk', 'snail']]
...
def main():
    ...
    for idx, split in enumerate(splits):
        ...
        open_class_to_idx = {}
        open_idx_to_class = {}
        open_idx_to_class[9] = 'cifar100'
        open_class_to_idx['cifar100'] = 9
...
def get_examples(file_path, image_folder, fold):
    ...
    coarse_class_names = meta['coarse_label_names']
    fine_class_names = meta['fine_label_names']
    ...
    for i in tqdm(range(len(filenames))):
        ...
        pixels = Image.fromarray(pixels)
        pixels.save(filename)
        fine_label = fine_class_names[fine_labels[i]]
        examples.append({
            'filename': filename,
            'data': pixels,
            'label': 9,
