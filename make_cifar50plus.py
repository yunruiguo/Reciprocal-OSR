    ...
    for idx, split in enumerate(splits):
        ...
        for item in temp_classes:
            if item not in cifar100_vehicles:
                ...
                if count >= 50:
                    break
        unknown_classes = sorted(unknown_labels)
        ...
        for i, e in enumerate(cifar100_examples):
            if e['label'] in unknown_classes:
                open_test_examples['filenames'] += [e['filename'].encode()]
                open_test_examples['data'] += [e['data']]
                open_test_examples['labels'] += [unknown_classes.index(e['label'])]
        open_meta_dict = {'image_size': 32, 'image_channels': 3}
        open_meta_dict['class_names'] = [cifar100_classes[i] for i in unknown_classes]
        open_class_to_idx = {}
        open_idx_to_class = {}
        for _idx, item in enumerate(open_meta_dict['class_names']):
